...
//
// A4's renderer test asserts only `%PDF-` magic + buffer length — a PDF whose
// every glyph is an empty .notdef box satisfies both, which is exactly how the
// 2026-06-17 F1 attorney sample shipped as rows of dots. This suite renders the
// brand typefaces to a real PDF and asserts the embedded font subsets carry
// actual drawable glyph outlines (see pdfGlyphProbe.js for the mechanism), and
// — for the fi/ff heading ligatures — that every letter paints.
//
// ── Why `@vitest-environment node` (this IS the notdef fix) ─────────────────
...
// jsdom/browser-build regression, a corrupt or missing registered font file, a
// registration that resolves to no drawable face, OR a heading ligature (fi/ff)
// that embeds empty so a letter drops ("Profle"/"Ofer").
// What this does NOT catch: correct glyphs at the wrong weight (a variable-axis
// fidelity concern, tracked separately) or content errors (the A4 suites).
...
// ── Heading ligature guard (the F1 "Personal Profle" / "Settlement Ofer" class) ──
// The display headings carry the only fi/ff ligatures in the document. Inter body
// ligatures ("benefit", "Inflation") render fine; the risk is a Playfair fi/ff/ffi
// ligature glyph embedding empty (so a letter drops). This renders a Playfair-only
// page packed with fi/fl/ff/ffi sequences plus the two real headings and asserts
// the Playfair subset paints EVERY glyph — only the standard .notdef may be empty.
function PlayfairLigatureDoc() {
  const st = { fontFamily: 'Playfair', fontWeight: 700, fontSize: 24 };
  return h(
    Document,
    null,
    h(
      ...
      h(Text, { style: st }, 'Personal Profile'),
      h(Text, { style: st }, 'Settlement Offer Overview'),
      h(Text, { style: st }, 'fi fl ff ffi ffl affluent baffle'),
    ),
  );
}

describe('heading ligature guard — fi/ff headings paint every letter (no dropped glyph)', () => {
  let pfSubsets;
  beforeAll(async () => {
    registerBlueprintFonts();
    const buffer = await renderToBuffer(h(PlayfairLigatureDoc));
    const summary = summarizeGlyphPainting(buffer);
    pfSubsets = summary.fonts.filter((f) => f.numGlyphs > 1);
  });

  it('embeds the Playfair subset', () => {
    expect(pfSubsets.length, JSON.stringify(pfSubsets)).toBeGreaterThanOrEqual(1);
  });

  it('paints every Playfair glyph used by "Profile"/"Offer"/fi·fl·ff·ffi — only .notdef may be empty', () => {
    // A correct render leaves exactly one empty glyph (the standard .notdef at
    // gid 0). A dropped/notdef ligature glyph would push emptyGlyphs to >=2 and
    // surface here ("Profle"/"Ofer").
    for (const f of pfSubsets) {
      expect(
        f.emptyGlyphs,
        `Playfair subset obj${f.obj} has ${f.emptyGlyphs} empty glyphs — a heading ligature likely dropped a letter. ${JSON.stringify(f)}`,
